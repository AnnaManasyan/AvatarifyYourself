### Place this file inside ECON/apps/
### This file enables the code to read AMASS sequences and animate
import argparse
import os
...
parser.add_argument("-n", "--name", type=str, default="back_img")
parser.add_argument("-m", "--motion", type=str, default="smpl_seq1")
...
smplx_pkl = np.load(f"./examples/motions/{args.motion}.npz", allow_pickle=True)
smplx_pose = torch.tensor(smplx_pkl['poses']).view(-1, 55, 3).float()
# remove the pose of eyes
smplx_pose[:, 23:23 + 2] *= 0.0 
smplx_transl = smplx_pkl['trans']
